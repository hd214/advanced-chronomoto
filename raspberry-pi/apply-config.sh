...
ARCHIVE_REFRESH_MS="${ARCHIVE_REFRESH_MS:-60000}"
# Enable analog clock in top-right (1 = enabled, 0 = disabled)
CLOCK_ENABLED="${CLOCK_ENABLED:-1}"
...
if [[ "$CLOCK_ENABLED" == "1" || "$CLOCK_ENABLED" == "true" ]]; then
  CLOCK_JS=true
else
  CLOCK_JS=false
fi
